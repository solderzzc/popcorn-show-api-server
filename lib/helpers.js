          ...
          if(!dateBased) {
            ...
            async.each(Object.keys(data), function(season, cb) {
              try {
                ...
                trakt.season(imdb , season, function(err, seasonData) {
                  for(var episodeData in seasonData){
                    ...
                    thisEpisode = {
                      ...
                      date_based: false,
                      first_aired: new Date(episodeData.first_aired).getTime()/1000,
            ...
            try {
              trakt.season(imdb,season , function(err, show) {
                ...
                if(!show) return callback(null, show);
                  ...
                  async.each(seasons, function(season, cbs) {
                    ...
                    async.each(episodes, function(episode, cbe){
                      var aired = new Date(episodeData.first_aired).getTime()/1000;
                      if(aired){
                ...
                try {
                  trakt.show(slug, function(err, data) {
                      if (!err && data) {
                            ...
                            trakt.showWatching(slug, function(err, data2) {
                                if (!err && data2) {
                                  ...
                                  Show.update({ _id: doc._id },
                                     ...
                                     images: { poster: data.images.poster.full,
                                                 fanart: data.images.fanart.full,
                                                 banner: data.images.fanart.full },
                                      rating: { watching: data2.length,
                                                percentage: Math.round(data.rating*10),
                                                votes: data.votes,
                                                loved: '0',
                                                hated: '0'
                                              }}},
